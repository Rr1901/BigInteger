...
class BigInteger {
    ...
    public:
        ...
        void setSign(bool s); //MVB for neg is 1 for pos is 0
        const bool& getSign();
...
int main() {
    ...
    BigInteger b1("100000000000000000000"),b2("100000000000000000000000");
    b1=b1.operator-(b2);
    cout<<b1.getSign();
    cout<<b1.getNumber();
    return 0;
...
//false for positive and true for neg
...
BigInteger BigInteger::operator - (BigInteger b) {
    b.setSign( ! b.getSign() ); // x - y = x + (-y) passing opposite sign for this only
    return (*this) + b; // called the + operator
}
...
BigInteger BigInteger::operator * (BigInteger b) {
    ...
    //  + -> false
    //- -> true
    //both opp sign then -(true)
    //both same sign +(false)
